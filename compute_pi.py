...
─────
  ...
  python compute_pi.py --input mydata.h5ad

  # Non-interactive (for pipelines)
  python compute_pi.py --input mydata.h5ad --r-threshold 0.3

  # Custom output path
  python compute_pi.py --input mydata.h5ad --output mydata_with_pi.h5ad
...
def _run(args, input_path: Path, output_path: Path, logger: RunLogger):
    ...
    if args.counts_layer is not None or args.libsize_key is not None:
        ...
        counts_layer = args.counts_layer          # may still be None → use X
    ...
    del counts  # free CC count matrix before full h5ad load
